...
const char *WAV_P_FmtString(uint16_t);
...
int WriteWavHeader(int fd, WAVContainer_t *wav_header);

time_t CopyRawDataToWAV(int wavFd, int pcmFd);

int FillWavFileHeader(WAVContainer_t *wav_header, int pcmFileLen, int m_sample);

extern int getSampleRate(int m_sample);
