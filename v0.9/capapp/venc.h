...
/*
	video encoder send output address to muxer not memcpy
*/
#define ENCODER_SEND_ONLY_ADDRESS_TO_MUXER

#ifdef HAVE_ANDROID_OS
//Caution: Don't exceed the number of queue allocated in the queue.h [MAX_QUEUE_ELEMENTS 100].
#define VIDEO_ENC_BUFFER_COUNT 20//150
...
#define VPU_ENCODER_CMD_INIT_FAILED		10001

#define VENC_PIC_TYPE_I	0
#define VENC_PIC_TYPE_P	1

#define	RATECONTROL_BITRATE		0x03
#define	RATECONTROL_FRAMERATE		0x05
#define	RATECONTROL_KEYFRAMEINTERVAL	0x09
...
typedef struct venc_init_t
{
	...
	unsigned int m_bChromaInterleaved;	/// 1:chroma interleaved 0:chroma saparate
	int m_iEncodingQualityLevel;		// 1 ~ 35
	int encoder_id;						// thread id(encoder identifier)
