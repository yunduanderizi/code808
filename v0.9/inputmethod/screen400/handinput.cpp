...
#define Text_Xstart  137
#define Text_Ystart  5
#define HandWriteAere_width 	273
#define HandWriteAere_heigth 	177
...
///extern bool tslib_enable;

HandInputWidget::HandInputWidget(QWidget *parent)
	: QWidget(parent),pix(QRect(Text_Xstart + 2 ,Text_Ystart + 2,HandWriteAere_width - 4,HandWriteAere_heigth - 4).size())
{
	/// tslib_enable=true;
...
//		paint.drawLine(lastX,lastY,mouseEvent->x(),mouseEvent->y());
...
{  
	...
	for (int i = 0; i < m_newPoints.size()-2 ; i+=2)
		{
			paint.drawLine(m_newPoints[i].x,m_newPoints[i].y,
					m_newPoints[i+2].x,m_newPoints[i+2].y);
		}

	if(m_newPoints.size()==2)
		paint.drawLine(m_newPoints[0].x,m_newPoints[0].y,
					m_newPoints[1].x,m_newPoints[1].y);
	
	paint.end();
	paint.begin(this);

	paint.drawPixmap(e->rect(),pix,e->rect());

/*
	if(m_newPoints.size()>1)
		{
	THWPoint pt1;
	pt1.x=m_newPoints[m_newPoints.size()-1].x;
	pt1.y=m_newPoints[m_newPoints.size()-1].y;
	m_newPoints.clear();
	m_newPoints.append(pt1);
	...
	/*
	for (int i = 0; i < m_arrPoints.size()-2 ; i++)
	{   
		if(( m_arrPoints[i].x == -1 &&  m_arrPoints[i].y == 0 ) || ( m_arrPoints[i+1].x == -1 &&  m_arrPoints[i+1].y == 0 ) )
		{
			continue;
		}
		else
		{
			paint.drawLine(m_arrPoints[i].x,m_arrPoints[i].y,
				m_arrPoints[i+1].x,m_arrPoints[i+1].y);   
		}
	}
	*/
}
...
{	    
    ...
    pbShow1->setGeometry(QRect(11, 11, tmp_width, tmp_height));
    ...
    pbShow2->setGeometry(QRect(75, 11, tmp_width, tmp_height));
    ...
    pbShow3->setGeometry(QRect(11, 53, tmp_width, tmp_height));
    ...
    pbShow4->setGeometry(QRect(75, 53, tmp_width, tmp_height));
    ...
    pbShow5->setGeometry(QRect(11, 95, tmp_width, tmp_height));
    ...
    pbShow6->setGeometry(QRect(75, 95, tmp_width, tmp_height));
    ...
    pbShow7->setGeometry(QRect(11, 137, tmp_width, tmp_height));
    ...
    pbShow8->setGeometry(QRect(75, 137, tmp_width, tmp_height));
    ...
    pbShow9->setGeometry(QRect(75, Text_Ystart, tmp_width, tmp_height));
    ...
    pbHide->setGeometry(QRect(415, 5, 60, 42));
    ...
    pbDel->setGeometry(QRect(415, 50, 60, 42));
    ...
    pbSpace->setGeometry(QRect(415, 95, 60, 42));
    ...
    pbChange->setGeometry(QRect(415, 140, 60, 42));
