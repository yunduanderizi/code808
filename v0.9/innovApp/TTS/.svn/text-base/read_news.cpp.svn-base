...
{
        /* 获取消息，用户实现 */
        if(ivTTS_Status_Play == ReadThread::object()->getRunStatus())
        {
                /* 继续合成 */
                return ivTTS_ERR_OK;
        }
...
{
        int err = 0;
        snd_pcm_t* handle=ReadThread::object()->getHandleValue();
        ...
        else if (err != (int)ReadThread::object()->getFramesValue() )
        {
...
ReadThread * ReadThread::thisObject = NULL;
ReadThread * ReadThread::object()
{
        if(thisObject==NULL)
        {
                thisObject = new ReadThread();
        }
        return thisObject;
}

void ReadThread::aisound_create()
{
        /* 设置状态 */
        run_status = ivTTS_Status_Idle;
...
//void ReadThread::aisound_start()
void ReadThread::aisound_start(const QString &content,int warn)
{
        m_quitMutex.lock();
        if(run_status ==  ivTTS_Status_Play)
        {
                run_status = ivTTS_Status_Exit;
                m_quitCond.wait(&m_quitMutex);
                wait();
        }

        contentToBeRead = content;
        run_status = ivTTS_Status_Play;
        warning_state = warn;
...
int ReadThread::aisound_start(int index,const QStringList &contentList,int warn)
{
        m_quitMutex.lock();
...
void ReadThread::aisound_play()
{
     ...
     //   ivReturn = ivTTS_SynthText(hTTS, ivText(readString), -1);
        if( isReadString && run_status == ivTTS_Status_Play)
        {
                ...
                if(isReadList)
                {
                        ...
                        {			        
                                ...
                                else
                                {
                                        ...
                                        if(ivTTS_Status_Exit == run_status)
                                        {
                                                break;
                                        }
                                }
        ...
        else if(isReadList && run_status == ivTTS_Status_Play)
        {
                for(;listIndex >=0 && listIndex < contentListToBeRead.count();listIndex++)
                {		        
                        if(warning_state)
                        ...
                        if(0 == ivReturn)
                        {
                        //    fprintf(stderr,"........diwei,line:%d,%s,%s,%s...........\n",__LINE__,__FUNCTION__,__FILE__,contentListToBeRead[listIndex].data());
                        //    fprintf(stderr,"........diwei,line:%d,%s,%s,%s...........\n",__LINE__,__FUNCTION__,__FILE__,contentListToBeRead[listIndex].toUtf8().data());
                                emit oneListReadend(listIndex);
                        }
                        else
                        {
...
int ReadThread::aisound_release()
{
        /* 逆初始化 */
...
int ReadThread::init_asound()
{
        int err = 0;

        fprintf(stderr,"snd_pcm_open\n");

        if ((err = snd_pcm_open(&handle, "plug:gps", SND_PCM_STREAM_PLAYBACK, 0)) < 0)
        {
...
int ReadThread::play_sign_sound()
{
        int err = 0;
...
void ReadThread::release_asound()
{
...
int ReadThread::check_read(int fd, void* buf)
{
        int char_num = 0;
        int sign_num = 0;
        char character[1];
...
ivTTS_Status ReadThread::setRunStatus(ivTTS_Status status)
{
        m_setMutex.lock();
        run_status = status;
        m_setMutex.unlock();
...
ivTTS_Status ReadThread::getRunStatus(void)
{
        return run_status;
}

int ReadThread::getBeingReadedIndex(void)
{
        return listIndex;
}

int ReadThread::setHandleValue(snd_pcm_t *h)
{
        m_setMutex.lock();
        handle = h;
        m_setMutex.unlock();
...
snd_pcm_t *ReadThread::getHandleValue(void)
{
        return handle;
}

snd_pcm_uframes_t ReadThread::getFramesValue(void)
{
        return frames;
}
