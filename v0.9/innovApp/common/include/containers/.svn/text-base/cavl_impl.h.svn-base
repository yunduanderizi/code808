...
/*
 * Initialization to 0 is not required but it pacifies compilers
 * as well as insure++
 */
#define L__BIT_ARR_DEFN(NAME) unsigned long NAME = 0;
...
L__SC_M void L__(init)(L__(tree) *L__tree) { L__tree->root = AVL_NULL; }

#endif

#if (L__IMPL_MASK & AVL_IMPL_IS_EMPTY)

L__SC int L__(is_empty)(L__(tree) *L__tree)
...
*/
L__SC AVL_HANDLE L__(balance)(L__(tree) *L__tree, AVL_HANDLE bal_h)
  {
    AVL_HANDLE deep_h;
...
L__SC_M AVL_HANDLE L__(insert)(L__(tree) *L__tree, AVL_HANDLE h)
  {
    ...
    if (L__tree->root == AVL_NULL){
    ...
    }else
      {
        ...
        ** so on. */
        ...
        if (unbal != AVL_NULL)
          {
            unbal = L__(balance)(L__tree, unbal);
            L__CHECK_READ_ERROR(AVL_NULL)
...
L__SC AVL_HANDLE L__(search)(L__(tree) *L__tree, AVL_KEY k, avl_search_type st)
  {
    int cmp, target_cmp;
...
#if (L__IMPL_MASK & AVL_IMPL_SEARCH_EQUAL)

/* Same as search but optimized when search type is AVL_EQUAL */
L__SC AVL_HANDLE L__(search_equal)(L__(tree) *L__tree, AVL_KEY k)
  {
    int cmp;
    AVL_HANDLE h = L__tree->root;
...
L__SC AVL_HANDLE L__(search_least)(L__(tree) *L__tree)
  {
    AVL_HANDLE h = L__tree->root;
    AVL_HANDLE parent = AVL_NULL;
...
#ifdef AVL_IMPL_SEARCH_GREATEST
#if (L__IMPL_MASK & AVL_IMPL_SEARCH_GREATEST)

L__SC AVL_HANDLE L__(search_greatest)(L__(tree) *L__tree)
  {
    AVL_HANDLE h = L__tree->root;
    AVL_HANDLE parent = AVL_NULL;
...
L__SC_M AVL_HANDLE L__(remove)(L__(tree) *L__tree, AVL_KEY k)
  {
    /* Zero-based depth in tree. */
...
L__SC_M AVL_HANDLE L__(subst)(L__(tree) *L__tree, AVL_HANDLE new_node)
  {
    AVL_HANDLE h = L__tree->root;
    AVL_HANDLE parent = AVL_NULL;
    int cmp, last_cmp;
...
L__SC int L__(build)(
  L__(tree) *L__tree, AVL_BUILD_ITER_TYPE p, L__SIZE num_nodes)
  {
...
L__SC void L__(start_iter)(
  L__(tree) *L__tree, L__(iter) *iter, AVL_KEY k, avl_search_type st)
  {
    AVL_HANDLE h = L__tree->root;
    unsigned d = 0;
    int cmp, target_cmp;
...
L__SC void L__(start_iter_least)(L__(tree) *L__tree, L__(iter) *iter)
  {
    AVL_HANDLE h = L__tree->root;
...
L__SC void L__(start_iter_greatest)(L__(tree) *L__tree, L__(iter) *iter)
  {
    AVL_HANDLE h = L__tree->root;
...
/*
 * walk function added by D. Pelle.
 * It it simpler to use than more generic iterator.
 * It is written in a recursive way because it is simpler this way.
 * Since tree is balanced and and since function does not have local
 * variables, stack usage should be very small anyway
 */
#if (L__IMPL_MASK & AVL_IMPL_WALK)
static int L__(endWalk);
L__(TWalkWorker) L__(walkWorker);

static void L__(walk_)(AVL_HANDLE h)
{ 
  if (h == AVL_NULL) { return; }

  L__(walk_)(AVL_GET_LESS(h, 1));
  if (L__(endWalk))           { return; };
  if (!(*L__(walkWorker))(h)) { L__(endWalk) = 1; return; }
  L__(walk_)(AVL_GET_GREATER(h, 1));
}

L__SC void L__(walk)(L__(tree) *L__tree,
                     L__(TWalkWorker) worker)
{
  L__(endWalk) = 0;
  L__(walkWorker) = worker;
  L__(walk_)(L__tree->root);
}
#endif
