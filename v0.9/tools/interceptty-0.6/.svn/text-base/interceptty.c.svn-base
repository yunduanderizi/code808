...
void *udp_recv_gnssmode_thread(void *param)
{
    int recvfd;
    ...
    char *cmd_gps = "$CFMOD,0*5F\r\n";  
    char *cmd_bd2 = "$CFMOD,1*5E\r\n";  
    char *cmd_gps_bd2 = "$CFMOD,4*5B\r\n";
    
    while(1)
    {
        FD_ZERO(&readudp);
        FD_SET(recvfd, &readudp); 
        retval = select(recvfd + 1, &readudp, NULL, NULL, NULL);

        if(retval)
        {
            recvfrom(recvfd, &gnss_mode, 4, 0, &client, (socklen_t *)&sin_len);
            printf("------udp gnss mode =%d---\n",gnss_mode);
            
            switch(gnss_mode)   
            {       
            case 1: 
                write(backfd[0], cmd_gps, strlen(cmd_gps)); 
                break;      
            case 2:                     
                write(backfd[0], cmd_bd2, strlen(cmd_bd2));         
                break;      
            case 3:                     
                write(backfd[0], cmd_gps_bd2, strlen(cmd_gps_bd2));         
                break;      
            default:            
                printf("error cmd!\n"); 
            }
