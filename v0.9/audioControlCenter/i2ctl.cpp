...
void help(void)
{
  ...
  fprintf(stdout, "Syntax: i2cset I2CBUS CHIP-ADDRESS DATA-ADDRESS VALUE [MODE]\n");
  fprintf(stdout, "  MODE is 'b[yte]' or 'w[ord]' (default b)\n");
  fprintf(stdout, "  I2CBUS is an integer\n");
  if((fptr = fopen("/proc/bus/i2c", "r"))) {
    fprintf(stdout, "  Installed I2C busses:\n");
...
int i2copen(int i2cbus)
{
 ...
 //         IBD_PRINTF(4,"Error: Could not open file `%s', `%s', or `%s': %s\n",filename1,filename2,filename3,strerror(ENOENT));
        }
        if (e1 != ENOENT) {
 //       IBD_PRINTF(4,"Error: Could not open file `%s' : %s\n",filename1,strerror(e1));
          if(e1 == EACCES)
		  ;
  //          IBD_PRINTF(4,"Run as root?\n");
        }
        if (e2 != ENOENT) {
...
//		IBD_PRINTF(4,"Error: Invalid mode!\n");
//		help();
		return -1;
	}
	if( (address<0) || (address>0x7f) ){
//		IBD_PRINTF(4,"Error: Chip address invalid!\n");
//		help();
		return -1;
	}
	if( (daddress<0) || (daddress>0xff) ){
//		IBD_PRINTF(4,"Error: Data address invalid!\n");
//		help();
		return -1;
	}
	if( (value<0) || ( (size==I2C_SMBUS_BYTE_DATA) && (value>0xff) ) || ( (size==I2C_SMBUS_WORD_DATA) && (value>0xffff) ) ){
//		IBD_PRINTF(4,"Error: Data value out of range!\n");
//		help();
		return -1;
	}
	/* check adapter functionality */
	if (ioctl(file,I2C_FUNCS,&funcs) < 0) {
//		IBD_PRINTF(4,"Error: Could not get the adapter functionality matrix: %s\n",strerror(errno));
//		exit(1);
		return -1;
	}
	switch(size) {
	...
	//			IBD_PRINTF(4, "Error: Adapter for i2c bus %d", i2cbus);
	//			IBD_PRINTF(4, " does not have byte write capability\n");
	//			exit(1);
				return -1;
			}
			break;
	...
	//			IBD_PRINTF(4, "Error: Adapter for i2c bus %d", i2cbus);
	//			IBD_PRINTF(4, " does not have word write capability\n");
	//			exit(1);
				return -1;
			}
			break;
...
//		exit(1);
	...
	//	IBD_PRINTF(4,"Error: Could not set address to %d: %s\n",address,strerror(errno));
	//	exit(1);
		return -1;
	}
	if(size == I2C_SMBUS_WORD_DATA) {
	...
	if(res<0) {
//		IBD_PRINTF(4, "Warning - readback failed\n");
		return -1;
	}
	return res;
}
...
void i2cclose(int file)
{
	...
	res = close(file);
//	if(res == 0)
//		IBD_PRINTF(4,"i2c device was closed");
//	else
//		IBD_PRINTF(4,"i2c device close failed");
}
