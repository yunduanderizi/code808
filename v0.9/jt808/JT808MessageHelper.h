...
typedef vector<string>ICNumString;
typedef enum
{
...
typedef enum
{
    ...
    JT808_TT_COM2= 0x42,
    JT808_TT_USERB= 0xF0,
	JT808_TT_PATH_DATA= 0xF2, //公交车路线数据
    JT808_TT_USERE= 0xFF,
    JT808_TT_USER8= 0xF8,
    JT808_TT_ACTIVATION= 0xF1,//渣土车下发ic 卡信息
}ENUM_JT808_TT_ID;
...
public:
	...
	static void push_deploy_para(JT808Packet & packet);
	static void push_loc_mileage(JT808Packet & packet);
	static void push_loc_mileage_to_mcu(JT808Packet & packet);
	static void  push_deploy_para_to_mcu(JT808Packet & packet);
	static void push_baseconfig_para_to_mcu(JT808Packet & packet);
    static void HandleRegisterResult(int result, string &auth);
    static void HandleCommonReply(int id, int result);
	static void Stringslice(string &utf_value,string &str, vector<string> *result);
   ...
   // static void HandleTermUpdate(JT808Packet &packet, int body, int len, unsigned short seq);
    ...
    static void HandleInfomationService(JT808Packet &packet, int body, int len, unsigned short seq);
    ...
    static void HandleParaSet(JT808Packet &packet, int body, int len, unsigned short seq);
	...
	static void  PackLocationreadfile(struct LocStatusData &locData,int seq_bak);

    static void HandleInfomationDemand(int sum, int flag, unsigned char buff[]);
	...
	static int GetParametersTOMcu(unsigned char * buf_para);
    ...
    static void SendAnswer(unsigned short seq, unsigned char answer,int time);
	...
	static void SendDregControl(unsigned short seq,int flag);
    ...
    static void SendTransmissionData_can(int type,int total, unsigned char *buf);
    ...
    static void SendBusDPassengerData(int itime, int index, char* name, int upcount, int dcount);
    static int SendBusDDriverData(char *filename, int stop_time, int m_num);
	static void SendBusDPosData(char *filename);
    static int ProcessBusDDownTransmit(JT808Packet &packet, int body, int size, unsigned short seq);
	static void HandleSystemRestFactory();
    static JT808MessageHandler *m_handler;
	static string GetSchemaDbFullName(string dbName)
    {
	...
	static string GetSingleAllName(string dbName)
    {
        char *path = getenv("INNOV_IMPORTANT_TABLE_DIR");
        if(path)
        {
            return path + dbName; 
        }
