...
void ConfigManager::Refresh()
{
    ...
    delete table;
    VDRDeployParaTable *deplytable = new VDRDeployParaTable();
    if(deplytable->GetStringValue((int)VDR_DEPLOY_PARA_AUTH_TOKEN, auth) < 0)
    {
	...
	memset(AuthString,0,64);
	memcpy(AuthString,auth.c_str(),auth.length());
	printf("AuthString   =============================%s              \n",AuthString);
    printf("m_mainServerAddress.c_str()==%s  , m_secondaryServerAddress.c_str()=%s  ,m_tcpPort  %d \n",m_mainServerAddress.c_str(),m_secondaryServerAddress.c_str(),m_tcpPort);
	...
	memset(Main_Server_To_Mcu,0,30);
	memset(Second_Server_To_Mcu,0,30);
	...
	Port_To_Mcu=m_tcpPort;
	printf("Main_Server_To_Mcu  %s ,Second_Server_To_Mcu.c_str()  %s \n",Main_Server_To_Mcu,Second_Server_To_Mcu);
