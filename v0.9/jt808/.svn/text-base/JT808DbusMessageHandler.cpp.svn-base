...
void JT808InfomationDemandHandler(DBusRequestC *request,DBusReplyC *&reply)
{
	...
	JT808InfomationDbusReqC *pReq = (JT808InfomationDbusReqC *)request;
	JT808MessageHelper::HandleInfomationDemand(pReq->sum,pReq->flag,pReq->buff);
	JT808InfomationDbusRepC *pReply = new JT808InfomationDbusRepC();
	reply=pReply;
	return;
}
#endif
...
{
    LogUtility::Log(LOG_LEVEL_DEBUG, "JT808TransmitServiceHandler get record request");
...
{
	JT808InfomationDbusNotifyC notify;
	notify.m_type = type;
	notify.m_len = len;
...
{
	JT808EventSetDbusNotifyC notify;
...
{
	JT808CommonEventReqC *pReq = (JT808CommonEventReqC *)request;
...
{
	JT808DregCommonReqC *pReq = (JT808DregCommonReqC *)request;
	int flag;
	if(pReq->type==1||pReq->type==2)
	{
		flag==pReq->type;
	}
	else if (pReq->type==3)
	...
	else if (pReq->value==2)
	{

	}
	else if (pReq->value==3)
	{
		
	}
	else if (pReq->value==8)
	{
