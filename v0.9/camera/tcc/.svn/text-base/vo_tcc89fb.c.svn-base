...
static unsigned int 	fb_window_pre[4] = {0, };
static unsigned int 	fb_addroffset_pre[2] = {0, };
long int screensize;
char *mediafb=NULL;
int phyfbmem;
//int test_phy;
...
static void writetomscfb(uint8_t *image[],  int src_w,int src_h,int dst_w,int dst_h)
{
	...
	M2M_ScalerForVideo_With_Address_noviqe(src_w, src_h, dst_w, dst_h, image[0], image[1], image[2], 
...
static int vo_need_viqe()
{	

	if(IsInterlace_VPU() /*&& get_fps_need_viqe()*/)
	{
		return 1;
	}
	else
...
void init_viqe_parm()
{
	viqe_exit=viqe_stop=viqe_wait=0;
}
...
static int config(uint32_t width, uint32_t height, uint32_t d_width, uint32_t d_height, uint32_t flags, char *title, uint32_t format)
{
 ...
 //	int vm   = flags & VOFLAG_MODESWITCHING;
	...
	tcc89buffer_size = 0;
  	...
  	tcc89src_fmt = format;
	#if 1 //fix scaler bug
	if((double)(tcc89dst_width/tcc89src_width)>4.0f)
	{
		tcc89dst_width=tcc89src_width*4;
	//	printf("scaler width overrun!!!\n");
	}
	if((double)(tcc89dst_height/tcc89src_height)>4.0f)
	{
		tcc89dst_height=tcc89src_height*4;
	//	printf("scaler height overrun!!!\n");
	}
	#endif
...
//	else
 //		tcc89mscflag=0;
	...
	tcc89viqelag=0;
	init_viqe_parm();
	config_viqe=vo_need_viqe();
	//config_viqe=0; //test
	if(!config_viqe) printf("no viqe because this video format is not intelaced!!!\n");
	else printf("open viqe to performance de-interlaced!!!\n");
...
//	unsigned int sizediff_of_H = 0; // difference of original size with scaler input size
  	...
  	switch (mpi->imgfmt) 
  	{
		...
		case IMGFMT_I420:
			...
			if(GetYuvMode()==0)  // VPU decoder
			{
				...
				image[3] = ReturnPhyAddr_prev(0);
				image[4] = ReturnPhyAddr_prev(1);
				image[5] = ReturnPhyAddr_prev(2);
			}
	...
	//		printf("tcc89mscfb:0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n",mpi->planes[0],mpi->planes[1],mpi->planes[2],image[0],image[1],image[2]);
			...
			if (tcc89mscflag == 1) 
			{
				...
				else //progressive video no need viqe
				{
					writetomscfb(image, tcc89src_width,tcc89src_height,tcc89dst_width, tcc89dst_height);
				}
