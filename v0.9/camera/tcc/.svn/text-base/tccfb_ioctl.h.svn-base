...
#define TCC_LCD_FB_IOCTL_GET_FORMAT				0x1E
/*----------------------------------------------------------------------------------------
 * [IN ]	unsigned int	[1]		- Ref to IMGCH_FMT_t Type
 *---------------------------------------------------------------------------------------*/
...
#define TCC_LCD_FB_IOCTL_GET_IMGWINDOW			0x1D
/*----------------------------------------------------------------------------------------
 * [IN ]	unsigned int	[4]		- [0]: Image start offset X
 *									  [1]: Image start offset Y
 *									  [2]: Image width
...
#define TCC_LCD_FB_IOCTL_SET_PAN_DISPLAY_OPT			0x53
/*----------------------------------------------------------------------------------------
 * [IN ]	unsigned int	[1]		- pandisplay option value
 *---------------------------------------------------------------------------------------*/
#define TCC_LCD_FB_IOCTL_GET_PAN_DISPLAY_OPT			0x54
/*----------------------------------------------------------------------------------------
 * [OUT]	unsigned int	[1]		- pandisplay option value
 *---------------------------------------------------------------------------------------*/
/*  [pandisplay option value]
 *  0 - setting饶, RU啊 set瞪锭 鳖瘤 扁促啡促啊 府畔. 亲惑 success.
 *  1 - RU 眉农窍咯 扁粮 Frame俊 措茄 RU啊 set登菌阑锭, setting. 酒聪搁, setting窍瘤 臼绊, 俊矾 府畔.(errno = EBUSY)
 *	2 - 公炼扒 setting. 亲惑 success.
 */
...
#define TCC_LCD_FB_IOCTL_SET_3D_MODE            	0x70
/*----------------------------------------------------------------------------------------
 * 
 *---------------------------------------------------------------------------------------*/

#define TCC_LCD_FB_IOCTL_SET_CHANGE_EYE           0x71
/*----------------------------------------------------------------------------------------
 * 
 *---------------------------------------------------------------------------------------*/

#define TCC_LCD_FB_IOCTL_SET_LEFT                       0x72
/*----------------------------------------------------------------------------------------
 * 
 *---------------------------------------------------------------------------------------*/

#define TCC_LCD_FB_IOCTL_SET_RIGHT                      0x73
/*----------------------------------------------------------------------------------------
 * 
 *---------------------------------------------------------------------------------------*/

#define TCC_LCD_FB_IOCTL_SET_3D_MODE_TEST                      0x74
/*----------------------------------------------------------------------------------------
 * 
 *---------------------------------------------------------------------------------------*/
 
#define TCC_LCD_FB_IOCTL_CAM_ONOFF      0x80
/*----------------------------------------------------------------------------------------
 * 
 *---------------------------------------------------------------------------------------*/

/*
*  下面的定义是为了支持两路输出视频而定义的控制常量和控制结构体
*  Cary
*  2010.12.09
*/
#define TCC_LCD_FB_IOCTL_DUAL_DISPLAY_LCD_CVBS	0x90
#define TCC_LCD_FB_IOCTL_DUAL_DISPLAY_MODE      0x91

//定义display_mode恒量，目前仅支持两种模式
#define DISP_MODE_DEFAULT 0	//原来的模式，为了兼容而定制此值
#define DISP_MODE_LCD_CVBS 1	//两路输出视频模式
typedef struct _tcc_dual_control {
	unsigned char display_mode; 
	unsigned char m2m_enable; // 1: fb0使用m2m输出到LCDC1.IMG0 0:不使用m2m
	unsigned char m2m_refresh;// 1: 刷新m2m的设置  0: 不刷新m2m设置
	int video_width_embedded; //嵌入的视频宽度
	int video_height_embedded;//嵌入的视频高度
	int video_xpos_embedded; //嵌入的视频顶点在x轴的位置
	int video_ypos_embedded;//嵌入的视频顶点在y轴的位置
	int video_width_src;
