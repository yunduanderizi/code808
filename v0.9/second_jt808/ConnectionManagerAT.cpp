...
void ConnectionManager::HandleTimer(string &name, void *)
{
	if(name == CONNECTION_TIMER_NAME)
	{
		...
		printf("Second Server  m_connId %d ConnectMainNum %d ConnectSecNum  %d \n",m_connId,ConnectMainNum,ConnectSecNum);
		Connect();
	}
}
void ConnectionManager::SleepConnect()
{
	Connect();
}
int ConnectionManager::Connect()
{
	...
	if (strcmp(serverAddress.c_str(),StrNull)==0)
	{
		...
		printf("Second Don't Set The serverAddress\n");
		return -1;
	}
	if (serverAddress.length()==0)
	{
		StartTimer(m_connectTimer);
		printf("Second Mian Don't Set The serverAddress  length  %d  \n",serverAddress.length());
		return -1;
	}
	...
	//sendReq((unsigned char *)req, sizeof(TcpReqHdr));
	//sendReq((unsigned char *)&(req->port), req->hdr.msglen);
...
int ConnectionManager::createServer()
{
	...
	memset(&addr, 0, sizeof(addr));
	addr.sun_family = AF_UNIX;
	len = sizeof(PHONE_DATA_SERVER_PATH3);
	memcpy(addr.sun_path, PHONE_DATA_SERVER_PATH3, len);
	addrlen = len + 2;
...
{
	printf("connected, %d  m_connected, %d\n", connected, m_connected);
	if(connected && !m_connected)
	{
		printf("DeadBlock_flag==%d \n",Lougout,DeadBlock_flag);
		if (DeadBlock_flag==0)
		{
	...
	{
		printf("Lougout==%d DeadBlock_flag==%d \n",Lougout,DeadBlock_flag);
		connect_tmp=0;
		if (Lougout==0)
		{
			if (DeadBlock_flag==0)
	...
	{
		printf(" Enter Wrong m_connected   %d    connected  %d  \n",m_connected,connected);
		if(!m_connected && !connected)
		{
			printf("Enter Wrong 2 m_connected   %d    connected  %d  \n",m_connected,connected);
		
			connect_tmp=0;
			jt808.m_authorized = false;  //chongxinfasong  jianquan 
			ConfigManager::SetAuthFlag(true);
			connect_num++;
			m_connectTimer->timeout=TIMER_SEC_TO_MSEC((connect_num+1)*CONNECT_TIME);
			StartTimer(m_connectTimer);
...
int ConnectionManager::Send(const unsigned char *buf, int len)
{
    ...
    printf("fei----------22222222222--------------JT8082Manager ----------------- FILE %s  LINE %d \n",__FILE__,__LINE__);
    for(int i = 0; i < len; i ++)
    {
        printf("%02x ", buf[i]);
        if((i + 1) % 16 == 0)
...
void ConnectionManager::NetBreak( int BreakNum)
{
	...
	sendReq((unsigned char *)req, sizeof(TcpReqConnect) + serverAddress.length());
	//sendReq((unsigned char *)req, sizeof(TcpReqHdr));
	//sendReq((unsigned char *)&(req->closelink), req->hdr.msglen);
